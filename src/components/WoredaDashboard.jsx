...
import { 
  ...
  FaUniversity, FaLandmark, FaCity, FaGlobe
} from 'react-icons/fa';
...
const WoredaDashboard = ({ user, onLogout }) => {
  ...
  const [showKebeleDetailModal, setShowKebeleDetailModal] = useState(false);
  const [selectedReport, setSelectedReport] = useState(null);
  const [selectedKebele, setSelectedKebele] = useState(null);
  const [expandedKebele, setExpandedKebele] = useState(null);
  const [kebeleHospitals, setKebeleHospitals] = useState({});
  ...
  const [recipients, setRecipients] = useState({ zones: [], kebeles: [] });
  ...
  useEffect(() => {
    ...
    socketRef.current.on('new_report_from_kebele', (data) => {
    ...
    socketRef.current.on('report_reply_from_kebele', (data) => {
  ...
  const fetchRecipients = async () => {
    try {
      ...
      setRecipients({ 
        ...
        kebeles: kebelesRes.data.kebeles || []
      });
    } catch (error) { 
      console.error('Error fetching recipients:', error);
      setRecipients({ zones: [], kebeles: [] });
    }
  };
  ...
  const fetchKebeleHospitals = async (kebeleId) => {
    ...
    try {
      ...
      setKebeleHospitals(prev => ({
        ...prev,
        [kebeleId]: {
          hospitals: hospitals,
          totalHospitals: hospitals.length,
          kebeleName: res.data.kebele_name
        }
      }));
      setExpandedKebele(kebeleId);
    } catch (error) {
      console.error('Error fetching kebele hospitals:', error);
      setKebeleHospitals(prev => ({
        ...prev,
        [kebeleId]: {
          hospitals: [],
          totalHospitals: 0,
          error: error.response?.data?.message || 'No hospitals found'
        }
      }));
      setExpandedKebele(kebeleId);
    } finally {
      setLoadingHospitals(prev => ({ ...prev, [kebeleId]: false }));
    }
  };
                    ...
                    >
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-gray-500">{kebele.email}</p>
                          <div className="flex gap-2 mt-1">
                            <span className={`text-xs px-2 py-0.5 rounded-full ${kebele.status === 'active' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-600'}`}>
                              {kebele.status}
                            </span>
                            <span className="text-xs text-gray-500">Code: {kebele.code}</span>
                          </div>
                        </div>
                      </div>
              ...
              <div>
                ...
                <div className="flex gap-4">
                  <label className="flex items-center gap-2">
                    ...
                    <FaGlobe className="text-purple-500" /> Zone Admin
                  </label>
                  <label className="flex items-center gap-2">
                    <input type="radio" value="kebele" checked={reportFormData.recipient_type === 'kebele'} onChange={(e) => setReportFormData({...reportFormData, recipient_type: e.target.value, recipient_id: ''})} />
                    <FaTree className="text-green-500" /> Kebele Admin
                  </label>
                </div>
              </div>
