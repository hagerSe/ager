...
const KebeleDashboard = ({ user, onLogout }) => {
  ...
  // Fetch STAFF DETAILS for a hospital
  ...
  const handleAttachmentSelect = async (e, isReply = false) => {
    ...
    for (const file of files) {
      ...
      try {
        ...
        if (res.data.success) {
          // ✅ IMPORTANT: Store ALL attachment data including the key
          const attachmentData = {
            ...
            key: res.data.file.key,      // ← CRITICAL: B2 file key
...
const downloadAttachment = async (attachment) => {
  try {
    ...
    if (!attachment.isValid && attachment.key === null) {
      alert('This file is from an old upload and cannot be downloaded. Please request a new upload.');
      return;
    }
    ...
    // Get the file key
    let fileKey = attachment.key;
    
    if (!fileKey || fileKey === 'undefined' || fileKey === 'null') {
      alert('Cannot download: file information is missing. This file may be from an old upload.');
      return;
    }
 ...
 const sendConversationReply = async () => {
  ...
  try {
    ...
    // ✅ ALWAYS send body (even if empty string)
    ...
    console.log('📤 Sending reply with:', {
      bodyLength: conversationReplyText.length,
    ...
    // ✅ This will show you the actual server error
    if (error.response) {
