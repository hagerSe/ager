...
import { 
  ...
  FaUniversity, FaLandmark, FaFilePowerpoint, FaFileExcel, FaFileWord,
  FaGlobe
} from 'react-icons/fa';
...
const KebeleDashboard = ({ user, onLogout }) => {
  ...
  // Hospital form data with validation
  ...
  const [hospitalFormErrors, setHospitalFormErrors] = useState({});
  ...
  const validateHospitalForm = () => {
    const errors = {};
    errors.hospital_name = validateName(hospitalFormData.hospital_name, 'Hospital name');
    errors.first_name = validateName(hospitalFormData.first_name, 'First name');
    errors.last_name = validateName(hospitalFormData.last_name, 'Last name');
    if (hospitalFormData.middle_name && hospitalFormData.middle_name.trim() !== '') {
      errors.middle_name = validateName(hospitalFormData.middle_name, 'Middle name');
    }
    errors.email = validateEmail(hospitalFormData.email);
    errors.age = validateAge(hospitalFormData.age);
    errors.password = validatePassword(hospitalFormData.password);
    errors.phone = validatePhone(hospitalFormData.phone);
    
    setHospitalFormErrors(errors);
  ...
  // Handle Hospital form input change with validation
  const handleHospitalInputChange = (e) => {
    const { name, value } = e.target;
    let processedValue = value;
    
    if (name === 'first_name' || name === 'last_name' || name === 'middle_name' || name === 'hospital_name') {
    ...
    setHospitalFormData({ ...hospitalFormData, [name]: processedValue });
    
    if (hospitalFormErrors[name]) {
      setHospitalFormErrors({ ...hospitalFormErrors, [name]: null });
    }
  };
  ...
  const fetchHospitalStaffDetails = async (hospitalId) => {
    ...
    try {
      ...
      const staffByDepartment = {
        Doctor: [], Nurse: [], Midwife: [], Pharma: [], Lab: [],
        Radio: [], Triage: [], Card_Office: [], Bed_Management: [], Human_Resource: []
      };
      ...
      setHospitalStaffDetails(prev => ({
        ...
        [hospitalId]: {
          ...
          staffByDepartment: {},
          totalStaff: 0,
          error: error.response?.data?.message || 'No staff found'
        }
      }));
      setExpandedHospital(hospitalId);
    } finally {
  ...
  const handleCreateHospital = async (e) => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        setHospitalFormErrors({});
        setShowHospitalModal(false);
        fetchDashboardData();
        alert('Hospital admin created successfully!');
      }
    } catch (error) { 
      alert(error.response?.data?.message || 'Error creating hospital'); 
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-100 to-blue-50 flex">
      ...
      >
        <div className="p-4">
          ...
          <nav className="space-y-1">
            ...
            <button 
              onClick={() => { setActiveTab('hospitals'); setShowConversationView(false); }} 
              className={`w-full flex items-center space-x-3 px-3 py-2.5 rounded-xl transition ${activeTab === 'hospitals' ? 'bg-gradient-to-r from-blue-600 to-cyan-600 shadow-lg' : 'hover:bg-blue-800'}`}
            >
              <FaHospital className="text-lg" /> {!sidebarCollapsed && <span className="text-sm font-medium">Hospitals</span>}
            </button>
            <button 
              onClick={() => { setActiveTab('reports'); setShowConversationView(false); fetchAllHospitalReports(); }} 
          ...
          <div className="px-6 py-4">
            <div className="flex justify-between items-center">
              <div>
                ...
                >
                  Welcome back, <span className="font-semibold text-gray-700">{profileData.first_name || user?.full_name || 'Admin'}</span> | {profileData.kebele_name} Kebele
                </motion.p>
              </div>
        ...
        <main className="flex-1 overflow-y-auto p-6">
              ...
              <div className="bg-gradient-to-r from-blue-600 to-cyan-600 rounded-2xl p-6 text-white shadow-lg">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    >
                      Manage hospitals, track reports, and monitor activities in {profileData.kebele_name} Kebele
                    </motion.p>
                  </div>
                </div>
              </div>
            ...
            >
              ...
              <div className="h-[500px] overflow-y-auto p-6 space-y-4 bg-gray-50">
                ...
                {conversationThread.map((msg, idx) => (
                  <motion.div 
                    key={msg.id} 
                    initial={{ opacity: 0, x: msg.sender_type === 'kebele' ? 50 : -50 }}
                    animate={{ opacity: 1, x: 0 }}
                    transition={{ delay: idx * 0.05 }}
                    className={`flex ${msg.sender_type === 'kebele' ? 'justify-end' : 'justify-start'}`}
                  >
                    <div className={`max-w-[70%] ${msg.sender_type === 'kebele' ? 'bg-gradient-to-r from-blue-600 to-cyan-600 text-white' : 'bg-white border shadow-sm'} rounded-2xl p-4`}>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-5 gap-5">
                {[
                  ...
                  { label: 'Hospitals', value: stats?.hospitals || 0, color: 'purple', icon: <FaHospital /> },
                  { label: 'Reports', value: allHospitalReports.length, color: 'green', icon: <FaChartBar /> },
            ...
            >
              <div className="flex justify-between items-center mb-6 flex-wrap gap-3">
                <h2 className="text-xl font-bold text-gray-800 flex items-center gap-2">
                  <FaChartBar className="text-blue-600" /> All Hospital Reports
                </h2>
                <button onClick={() => fetchAllHospitalReports()} className="px-4 py-2 bg-gradient-to-r from-blue-600 to-cyan-600 text-white rounded-xl hover:shadow-lg transition text-sm font-medium flex items-center gap-2">
                  <FaSync /> Refresh
                </button>
              </div>
              ...
              {allHospitalReports.length > 0 ? (
                <div className="space-y-4">
                  {allHospitalReports.map((report, idx) => (
                    <motion.div 
                      key={report.id} 
                      initial={{ opacity: 0, x: -20 }}
                      animate={{ opacity: 1, x: 0 }}
          ...
          {activeTab === 'hospitals' && !showConversationView && (
            ...
            >
              <div className="flex justify-between items-center mb-6 flex-wrap gap-3">
                <h2 className="text-xl font-bold text-gray-800 flex items-center gap-2">
                  <FaHospital className="text-blue-600" /> Hospitals Management
                </h2>
                <div className="flex gap-3">
                  ...
                  <button onClick={() => setShowHospitalModal(true)} className="px-4 py-2 bg-gradient-to-r from-blue-600 to-cyan-600 text-white rounded-xl hover:shadow-lg transition text-sm font-medium flex items-center gap-2">
                    <FaPlus /> Add Hospital
                  </button>
                </div>
              </div>
              ...
              <div className="space-y-4">
                {hospitals.length === 0 && (
                  <div className="text-center py-12">
                    ...
                    <button onClick={() => setShowHospitalModal(true)} className="mt-4 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">
                      ...
                      <div className="flex items-center gap-3">
                        {hospitalStaffDetails[hospital.id] && (
                          <div className="text-right">
                            <p className="text-sm font-bold text-blue-600">{hospitalStaffDetails[hospital.id].totalStaff} Total Staff</p>
                          </div>
                        )}
                      ...
                      >
                        <div className="flex flex-wrap gap-2 mb-4 pb-2 border-b">
                          <button 
                            ...
                            className={`px-3 py-1.5 text-sm rounded-lg transition flex items-center gap-1 ${activeStaffDept === 'all' ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}`}
                          >
                          ...
                          {Object.entries(hospitalStaffDetails[hospital.id].staffByDepartment).map(([dept, staffList]) => (
                            staffList.length > 0 && (
                              <button 
                                ...
                                className={`px-3 py-1.5 text-sm rounded-lg transition flex items-center gap-1 ${activeStaffDept === dept ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}`}
                              >
                                {getDepartmentIcon(dept)} {dept.replace('_', ' ')} ({staffList.length})
                              </button>
                            )
                          ))}
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 max-h-96 overflow-y-auto">
                          ...
                          {(activeStaffDept === 'all' ? hospitalStaffDetails[hospital.id]?.allStaff : hospitalStaffDetails[hospital.id]?.staffByDepartment[activeStaffDept] || []).map((staff, sIdx) => (
                            <motion.div 
                              key={staff.id} 
                              initial={{ opacity: 0, scale: 0.95 }}
                              animate={{ opacity: 1, scale: 1 }}
                                ...
                                <div className="flex-1 min-w-0">
                                  ...
                                  <div className="flex items-center gap-2 mt-1 flex-wrap">
                                    <span className="text-xs px-2 py-0.5 rounded-full bg-teal-100 text-teal-700">
                                      {staff.department?.replace('_', ' ') || 'Staff'}
                                    </span>
                                    {staff.position && (
                                  ...
                                  <div className="mt-2 text-xs text-gray-500 space-y-0.5">
                                    ...
                                    {staff.license_number && <p>🪪 License: {staff.license_number}</p>}
                                  </div>
                                </div>
                              </div>
                            </motion.div>
              ...
              <div className="bg-gradient-to-r from-blue-600 to-cyan-600 px-8 py-10">
                <div className="flex items-center gap-6">
                  ...
                  <div className="text-white">
                    ...
                    <p className="text-blue-100">{profileData.kebele_name} Kebele</p>
                  </div>
                </div>
              </div>
              <div className="p-8">
      ...
      {/* Add Hospital Modal with Validation */}
      <AnimatePresence>
        {showHospitalModal && (
          <motion.div 
            initial={{ opacity: 0 }}
          ...
          >
            ...
            >
              <div className="sticky top-0 bg-white border-b px-6 py-4 flex justify-between items-center">
                <h2 className="text-xl font-bold text-gray-800">Add Hospital Admin</h2>
                <button onClick={() => setShowHospitalModal(false)} className="p-2 hover:bg-gray-100 rounded-full text-2xl">×</button>
              </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="md:col-span-2">
                    ...
                    <input 
                      type="text" 
                      name="hospital_name"
                      placeholder="e.g., Addis Hospital" 
                      value={hospitalFormData.hospital_name} 
                      onChange={handleHospitalInputChange}
                      className={`w-full px-4 py-3 border rounded-xl text-sm focus:ring-2 focus:ring-blue-500 ${hospitalFormErrors.hospital_name ? 'border-red-500' : 'border-gray-300'}`}
                      required 
                    />
                    {hospitalFormErrors.hospital_name && <p className="text-red-500 text-xs mt-1">{hospitalFormErrors.hospital_name}</p>}
                  </div>
                  ...
                  <div>
                    ...
                    <input 
                      ...
                      onChange={handleHospitalInputChange}
                      className={`w-full px-4 py-3 border rounded-xl text-sm focus:ring-2 focus:ring-blue-500 ${hospitalFormErrors.first_name ? 'border-red-500' : 'border-gray-300'}`}
                      required 
                    />
                    {hospitalFormErrors.first_name && <p className="text-red-500 text-xs mt-1">{hospitalFormErrors.first_name}</p>}
                  </div>
                  ...
                  <div>
                    ...
                    <input 
                      ...
                      onChange={handleHospitalInputChange}
                      className={`w-full px-4 py-3 border rounded-xl text-sm focus:ring-2 focus:ring-blue-500 ${hospitalFormErrors.middle_name ? 'border-red-500' : 'border-gray-300'}`}
                    />
                    {hospitalFormErrors.middle_name && <p className="text-red-500 text-xs mt-1">{hospitalFormErrors.middle_name}</p>}
                  </div>
                  ...
                  <div>
                    ...
                    <input 
                      ...
                      onChange={handleHospitalInputChange}
                      className={`w-full px-4 py-3 border rounded-xl text-sm focus:ring-2 focus:ring-blue-500 ${hospitalFormErrors.last_name ? 'border-red-500' : 'border-gray-300'}`}
                      required 
                    />
                    {hospitalFormErrors.last_name && <p className="text-red-500 text-xs mt-1">{hospitalFormErrors.last_name}</p>}
                  </div>
                  ...
                  <div>
                    ...
                    <input 
                      ...
                      min="18"
                      max="100"
                      className={`w-full px-4 py-3 border rounded-xl text-sm focus:ring-2 focus:ring-blue-500 ${hospitalFormErrors.age ? 'border-red-500' : 'border-gray-300'}`}
                      required 
                    />
                    {hospitalFormErrors.age && <p className="text-red-500 text-xs mt-1">{hospitalFormErrors.age}</p>}
                  </div>
                  ...
                  <div className="md:col-span-2">
                    ...
                    <input 
                      ...
                      onChange={handleHospitalInputChange}
                      className={`w-full px-4 py-3 border rounded-xl text-sm focus:ring-2 focus:ring-blue-500 ${hospitalFormErrors.email ? 'border-red-500' : 'border-gray-300'}`}
                      required 
                    />
                    {hospitalFormErrors.email && <p className="text-red-500 text-xs mt-1">{hospitalFormErrors.email}</p>}
                  ...
                  <div>
                    ...
                    <input 
                      ...
                      onChange={handleHospitalInputChange}
                      className={`w-full px-4 py-3 border rounded-xl text-sm focus:ring-2 focus:ring-blue-500 ${hospitalFormErrors.phone ? 'border-red-500' : 'border-gray-300'}`}
                    />
                    {hospitalFormErrors.phone && <p className="text-red-500 text-xs mt-1">{hospitalFormErrors.phone}</p>}
                  ...
                  <div>
                    ...
                    <input 
                      ...
                      onChange={handleHospitalInputChange}
                      className={`w-full px-4 py-3 border rounded-xl text-sm focus:ring-2 focus:ring-blue-500 ${hospitalFormErrors.password ? 'border-red-500' : 'border-gray-300'}`}
                      required 
                      minLength="6"
                    />
                    {hospitalFormErrors.password && <p className="text-red-500 text-xs mt-1">{hospitalFormErrors.password}</p>}
                  </div>
                </div>
                <div className="flex justify-end gap-3 pt-4 border-t">
                  <button type="button" onClick={() => setShowHospitalModal(false)} className="px-5 py-2.5 border border-gray-300 rounded-xl hover:bg-gray-50 transition">
                    Cancel
                  </button>
              ...
              <div>
                ...
                <div className="flex gap-4">
                  ...
                  <label className="flex items-center gap-2">
                    ...
                    <FaGlobe className="text-red-500" /> Woreda Admin
                  </label>
                </div>
              </div>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full p-6">
            <div className="flex justify-between items-center mb-4">
              ...
              <button onClick={() => { setShowHospitalDetailModal(false); setSelectedHospital(null); }} className="p-2 hover:bg-gray-100 rounded-full text-2xl">×</button>
            </div>
            <div className="flex items-center gap-4 mb-6">
              ...
              <div>
                <h3 className="text-lg font-bold text-gray-800">{selectedHospital.name}</h3>
                <p className="text-blue-600 text-sm">{selectedHospital.service_type || 'Public'} Hospital</p>
              </div>
            </div>
            <div className="bg-gray-50 rounded-xl p-5">
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div><p className="text-gray-500 text-xs">Admin Name</p><p className="font-medium text-gray-800">{selectedHospital.admin_name}</p></div>
                <div><p className="text-gray-500 text-xs">Email</p><p className="font-medium text-gray-800">{selectedHospital.admin_email}</p></div>
                <div><p className="text-gray-500 text-xs">Phone</p><p className="font-medium text-gray-800">{selectedHospital.admin_phone || 'Not provided'}</p></div>
                <div><p className="text-gray-500 text-xs">Status</p><p className={`font-medium ${selectedHospital.status === 'active' ? 'text-green-600' : 'text-gray-600'}`}>{selectedHospital.status}</p></div>
              </div>
            </div>
          </div>
        </div>
