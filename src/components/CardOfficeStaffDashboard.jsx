...
const TriageDashboard = ({ user, onLogout }) => {
  // ==================== HELPER: Get Hospital ID (ROBUST - LIKE CARD OFFICE) ====================
  const getHospitalId = () => {
    console.log('=== TRIAGE USER OBJECT DEBUG ===');
    console.log('Full user object:', user);
    console.log('user?.hospital_id:', user?.hospital_id);
    console.log('user?.hospitalId:', user?.hospitalId);
    
    // Try multiple sources
    let id = user?.hospital_id || user?.hospitalId || localStorage.getItem('hospital_id');
    ...
    if (!id) {
      try {
        ...
        if (storedUser) {
          ...
          id = parsed?.hospital_id || parsed?.hospitalId;
  ...
  // ==================== SCHEDULE STATES (ADDED - LIKE CARD OFFICE) ====================
  ...
  // ==================== GET SHIFT DISPLAY (ADDED - LIKE CARD OFFICE) ====================
  ...
  // ==================== SCHEDULE FUNCTION (ADDED - LIKE CARD OFFICE) ====================
  ...
  // ==================== FETCH DATA (FIXED - NO /api/ prefix) ====================
  ...
  // ==================== REPORT FUNCTIONS (FIXED - NO /api/ prefix) ====================
  ...
  // ==================== PROFILE FUNCTIONS (FIXED - NO /api/ prefix) ====================
  ...
  // ==================== INITIAL LOAD (FIXED - WITH SCHEDULE) ====================
  useEffect(() => {
    const hospitalId = getHospitalId();
    if (!hospitalId) {
    ...
    fetchHospitalAdmins();
    fetchProfile();
    fetchMySchedule(); // ← ADDED - LIKE CARD OFFICE
    ...
    <div className={`min-h-screen bg-gradient-to-br from-teal-50 to-emerald-50 flex ${textSizeClasses.base}`}>
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="max-w-[1600px] mx-auto p-10">
          ...
          {/* Schedule View Tab - COMPLETE VERSION LIKE CARD OFFICE */}
          {showScheduleView && (
            <div className="bg-white rounded-2xl shadow-md border border-gray-100 p-6">
              <div className="flex justify-between items-center mb-6">
                <div className="flex items-center gap-3">
