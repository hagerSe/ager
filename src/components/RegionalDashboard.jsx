...
const RegionalDashboard = ({ user, onLogout }) => {
  ...
  const [recipients, setRecipients] = useState({ federal: null, zones: [] });
  ...
  useEffect(() => {
    ...
    socketRef.current.on('new_report_from_zone', (data) => {
    ...
    socketRef.current.on('report_reply_from_zone', (data) => {
    ...
    socketRef.current.on('new_report_from_federal', (data) => {
      setRealTimeNotification({ id: Date.now(), type: 'report', title: 'New Report from Federal', message: `${data.sender_name} sent: "${data.title}"`, priority: data.priority, timestamp: new Date() });
      setTimeout(() => setRealTimeNotification(null), 6000);
      fetchDashboardData();
    });
  ...
  const fetchProfile = async () => {
    try {
      ...
      if (res.data.success) {
        ...
        setProfileData({
          first_name: regional.first_name || '', middle_name: regional.middle_name || '', last_name: regional.last_name || '',
          gender: regional.gender || '', age: regional.age || '', phone: regional.phone || '', email: regional.email || '',
          region_name: regional.region_name || '', address: regional.address || ''
        });
      }
  ...
  const fetchRecipients = async () => {
    try {
      ...
      setRecipients({ federal: null, zones: [] });
    }
  };
  ...
  const fetchZoneWoredas = async (zoneId) => {
    ...
    try {
      ...
      const woredas = res.data.woredas || [];
      ...
      console.error('Error fetching zone woredas:', error);
      setZoneWoredas(prev => ({
        ...prev,
        [zoneId]: {
          woredas: [],
          totalWoredas: 0,
          error: error.response?.data?.message || 'No woredas found'
        }
      }));
      setExpandedZone(zoneId);
    } finally {
      setLoadingWoredas(prev => ({ ...prev, [zoneId]: false }));
    }
  };
