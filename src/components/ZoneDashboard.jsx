...
const ZoneDashboard = ({ user, onLogout }) => {
  ...
  // Fetch recipients (Regional and Woredas)
  ...
  const handleCreateWoreda = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await axios.post(`${API_URL}/api/zone/woredas`, woredaFormData, { headers: { Authorization: `Bearer ${token}` } });
      if (res.data.success) {
    ...
    } catch (error) { alert(error.response?.data?.message || 'Error creating woreda'); }
  };
  ...
  const updateProfile = async () => {
    try {
      const token = localStorage.getItem('token');
      const res = await axios.put(`${API_URL}/api/zone/profile`, profileData, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const changePassword = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await axios.put(`${API_URL}/api/zone/change-password`, { current_password: passwordData.current_password, new_password: passwordData.new_password }, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const markNotificationAsRead = async (id) => {
    try {
      const token = localStorage.getItem('token');
      await axios.put(`${API_URL}/api/zone/notifications/${id}/read`, {}, { headers: { Authorization: `Bearer ${token}` } });
      fetchDashboardData();
  ...
  const markAllAsRead = async () => {
    try {
      const token = localStorage.getItem('token');
      await axios.put(`${API_URL}/api/zone/notifications/read-all`, {}, { headers: { Authorization: `Bearer ${token}` } });
      fetchDashboardData();
  ...
  const markReportAsRead = async (reportId) => {
    try {
      const token = localStorage.getItem('token');
      await axios.put(`${API_URL}/api/zone/reports/${reportId}/read`, {}, { headers: { Authorization: `Bearer ${token}` } });
      fetchDashboardData();
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-purple-50 to-indigo-50">
        <div className="text-center"><FaSpinner className="animate-spin text-3xl text-purple-600 mx-auto mb-3" /><p className="text-gray-600">Loading Dashboard...</p></div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-50 to-indigo-50 flex">
      ...
      {/* Sidebar */}
      <div className={`bg-gradient-to-b from-purple-900 to-purple-800 text-white transition-all duration-300 ${sidebarCollapsed ? 'w-20' : 'w-64'} shadow-2xl`}>
        <div className="p-4">
          <div className="flex items-center justify-between mb-8">
            {!sidebarCollapsed && (
              <div className="flex items-center gap-2">
                <div className="w-8 h-8 bg-gradient-to-r from-purple-500 to-indigo-500 rounded-lg flex items-center justify-center"><FaCity className="text-white text-sm" /></div>
                <span className="font-bold text-base">Zone Admin</span>
              </div>
            )}
            {sidebarCollapsed && <div className="w-8 h-8 bg-gradient-to-r from-purple-500 to-indigo-500 rounded-lg flex items-center justify-center mx-auto"><FaCity className="text-white text-sm" /></div>}
            <button onClick={() => setSidebarCollapsed(!sidebarCollapsed)} className="p-2 hover:bg-purple-700 rounded-lg">{sidebarCollapsed ? <FaChevronRight /> : <FaChevronLeft />}</button>
          </div>
          <nav className="space-y-1">
            <button onClick={() => { setActiveTab('dashboard'); setShowConversationView(false); }} className={`w-full flex items-center space-x-3 px-3 py-2 rounded-xl transition ${activeTab === 'dashboard' ? 'bg-gradient-to-r from-purple-600 to-indigo-600 shadow-lg' : 'hover:bg-purple-700'}`}>
              <FaHome className="text-lg" /> {!sidebarCollapsed && <span>Dashboard</span>}
            </button>
            <button onClick={() => { setActiveTab('woredas'); setShowConversationView(false); }} className={`w-full flex items-center space-x-3 px-3 py-2 rounded-xl transition ${activeTab === 'woredas' ? 'bg-gradient-to-r from-purple-600 to-indigo-600 shadow-lg' : 'hover:bg-purple-700'}`}>
              <FaMapMarkerAlt className="text-lg" /> {!sidebarCollapsed && <span>Woredas</span>}
            </button>
            <button onClick={() => { setActiveTab('inbox'); fetchDashboardData(); setShowConversationView(false); }} className={`w-full flex items-center space-x-3 px-3 py-2 rounded-xl transition relative ${activeTab === 'inbox' ? 'bg-gradient-to-r from-purple-600 to-indigo-600 shadow-lg' : 'hover:bg-purple-700'}`}>
            ...
            <button onClick={() => { setActiveTab('outbox'); setShowConversationView(false); }} className={`w-full flex items-center space-x-3 px-3 py-2 rounded-xl transition ${activeTab === 'outbox' ? 'bg-gradient-to-r from-purple-600 to-indigo-600 shadow-lg' : 'hover:bg-purple-700'}`}>
              <FaPaperPlane className="text-lg" /> {!sidebarCollapsed && <span>Sent Reports</span>}
            </button>
            <button onClick={() => { setActiveTab('profile'); setShowConversationView(false); }} className={`w-full flex items-center space-x-3 px-3 py-2 rounded-xl transition ${activeTab === 'profile' ? 'bg-gradient-to-r from-purple-600 to-indigo-600 shadow-lg' : 'hover:bg-purple-700'}`}>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="px-6 py-4">
            <div className="flex justify-between items-center">
              <div>
                ...
                <p className="text-xs text-gray-500">Welcome back, {profileData.first_name || user?.full_name || 'Admin'} | {profileData.zone_name} Zone</p>
              </div>
            ...
            <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}
              ...
              <div className="max-h-80 overflow-y-auto">
                {notifications.length > 0 ? notifications.map((notif) => (
                  <div key={notif.id} className={`p-4 border-b border-gray-100 hover:bg-gray-50 cursor-pointer ${!notif.is_read ? 'bg-purple-50' : ''} ${notif.priority === 'urgent' ? 'border-l-4 border-red-500' : ''}`} onClick={() => markNotificationAsRead(notif.id)}>
        ...
        <main className="flex-1 overflow-y-auto p-6">
          ...
          {showConversationView && (
            <div className="bg-white rounded-2xl shadow-md overflow-hidden">
              <div className="bg-gradient-to-r from-purple-600 to-indigo-600 px-6 py-4 flex justify-between items-center">
                <button onClick={() => { setShowConversationView(false); setConversationThread([]); setCurrentConversationId(null); }} className="text-white hover:text-purple-200 flex items-center gap-2">
                  <FaArrowLeft /> Back
                </button>
              ...
              <div className="h-[500px] overflow-y-auto p-6 space-y-4 bg-gray-50">
                ...
                {conversationThread.map((msg) => (
                  <div key={msg.id} className={`flex ${msg.sender_type === 'zone' ? 'justify-end' : 'justify-start'}`}>
                    <div className={`max-w-[70%] ${msg.sender_type === 'zone' ? 'bg-gradient-to-r from-purple-600 to-indigo-600 text-white' : 'bg-white border'} rounded-2xl p-4 shadow-sm`}>
                ...
                <div className="flex gap-3">
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <button
                      ...
                      className="p-3 bg-gradient-to-r from-purple-600 to-indigo-600 text-white rounded-xl hover:shadow-lg disabled:opacity-50"
                    >
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-5 gap-5">
                <div className="bg-white rounded-2xl p-5 shadow-md">
                  <div className="flex justify-between">
                    <div><p className="text-sm text-gray-500">Inbox</p><p className="text-3xl font-bold text-purple-600">{stats?.inbox || 0}</p></div>
                    <div className="w-12 h-12 bg-purple-100 rounded-xl flex items-center justify-center"><FaInbox className="text-xl text-purple-600" /></div>
                  </div>
                </div>
                <div className="bg-white rounded-2xl p-5 shadow-md">
                  <div className="flex justify-between">
                    <div><p className="text-sm text-gray-500">Outbox</p><p className="text-3xl font-bold text-indigo-600">{stats?.outbox || 0}</p></div>
                    <div className="w-12 h-12 bg-indigo-100 rounded-xl flex items-center justify-center"><FaPaperPlane className="text-xl text-indigo-600" /></div>
                  </div>
                </div>
                <div className="bg-white rounded-2xl p-5 shadow-md">
                  <div className="flex justify-between">
                    <div><p className="text-sm text-gray-500">Woredas</p><p className="text-3xl font-bold text-blue-600">{stats?.totalWoredas || 0}</p></div>
                    <div className="w-12 h-12 bg-blue-100 rounded-xl flex items-center justify-center"><FaMapMarkerAlt className="text-xl text-blue-600" /></div>
                  </div>
                </div>
                <div className="bg-white rounded-2xl p-5 shadow-md">
                  <div className="flex justify-between">
                ...
                <div className="bg-white rounded-2xl p-5 shadow-md">
                  <div className="flex justify-between">
                    <div><p className="text-sm text-gray-500">Urgent</p><p className="text-3xl font-bold text-red-600">{urgentCount}</p></div>
                    <div className="w-12 h-12 bg-red-100 rounded-xl flex items-center justify-center"><FaExclamationTriangle className="text-xl text-red-600" /></div>
                  </div>
                </div>
              </div>
              <div className="bg-white rounded-2xl p-6 shadow-md">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                  <button onClick={() => { setShowReportModal(true); fetchRecipients(); }} className="p-4 bg-gradient-to-r from-purple-600 to-indigo-600 text-white rounded-xl hover:shadow-lg text-sm font-medium flex items-center justify-center gap-2"><FaPaperPlane /> New Report</button>
                  <button onClick={() => setShowWoredaModal(true)} className="p-4 bg-gradient-to-r from-blue-600 to-cyan-600 text-white rounded-xl hover:shadow-lg text-sm font-medium flex items-center justify-center gap-2"><FaPlus /> Add Woreda</button>
                  <button onClick={() => setActiveTab('woredas')} className="p-4 bg-gradient-to-r from-green-600 to-emerald-600 text-white rounded-xl hover:shadow-lg text-sm font-medium flex items-center justify-center gap-2"><FaMapMarkerAlt /> View Woredas</button>
          ...
          {/* WOREDAS TAB - Each woreda with their kebeles */}
          {activeTab === 'woredas' && !showConversationView && (
            <div className="bg-white rounded-2xl shadow-md p-6">
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-xl font-bold text-gray-800">🏢 Woreda Administration</h2>
                <div className="flex gap-3">
                  <div className="relative">
                    <input type="text" placeholder="Search woredas..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-10 pr-4 py-2 border rounded-xl text-sm focus:ring-2 focus:ring-purple-500 w-64" />
                    <FaSearch className="absolute left-3 top-3 text-gray-400 text-sm" />
                  </div>
                  <button onClick={() => setShowWoredaModal(true)} className="px-4 py-2 bg-gradient-to-r from-purple-600 to-indigo-600 text-white rounded-xl hover:shadow-lg text-sm font-medium flex items-center gap-2"><FaPlus /> Add Woreda</button>
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                {woredas.map((woreda) => (
                  <div key={woreda.id} className="border rounded-xl overflow-hidden">
                    {/* Woreda Header - Click to expand and see kebeles */}
                    <div 
                      className="bg-gradient-to-r from-purple-50 to-indigo-50 p-5 cursor-pointer hover:from-purple-100 hover:to-indigo-100 transition flex justify-between items-center"
                    ...
                    {/* Expanded Content - Kebeles under this woreda */}
                      ...
                      <div className="p-5 bg-gray-50 border-t">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                          {woredaKebeles[woreda.id].kebeles.map((kebele) => (
                            <div key={kebele.id} className="bg-white rounded-xl p-4 shadow-sm border hover:shadow-md transition">
                              <div className="flex items-start gap-3">
                        ...
                        <div className="flex justify-end mt-4 pt-4 border-t">
                          <button 
                            ...
                            className="px-4 py-2 text-sm bg-gradient-to-r from-purple-600 to-indigo-600 text-white rounded-lg hover:shadow-md flex items-center gap-2"
                          >
          ...
          {activeTab === 'inbox' && !showConversationView && (
            <div className="bg-white rounded-2xl shadow-md p-6">
              <div className="flex justify-between items-center mb-6">
                ...
                <button onClick={() => { setShowReportModal(true); fetchRecipients(); }} className="px-4 py-2 bg-gradient-to-r from-purple-600 to-indigo-600 text-white rounded-xl hover:shadow-lg text-sm font-medium flex items-center gap-2"><FaPlus /> New Report</button>
              </div>
              <div className="space-y-4">
                {inbox.length === 0 && (
                ...
                {inbox.map((report) => (
                  <div key={report.id} className={`border rounded-xl p-4 cursor-pointer hover:shadow-md transition ${!report.is_opened ? 'border-purple-300 bg-purple-50' : 'border-gray-200 bg-white'}`} onClick={() => viewReportDetails(report)}>
            ...
            <div className="bg-white rounded-2xl shadow-md overflow-hidden">
              <div className="bg-gradient-to-r from-purple-600 to-indigo-600 px-8 py-10">
                <div className="flex items-center gap-6">
                  <div className="relative"><div className="w-24 h-24 bg-white rounded-full flex items-center justify-center shadow-xl"><FaUserCircle className="text-purple-600 text-6xl" /></div></div>
                  <div className="text-white"><h2 className="text-2xl font-bold">{profileData.first_name} {profileData.last_name}</h2><p className="text-purple-100">{profileData.zone_name} Zone</p></div>
                </div>
              </div>
              <div className="p-8">
                <div className="flex justify-between items-center mb-6"><h3 className="text-lg font-bold text-gray-800">Zone Administrator Information</h3>{!isEditingProfile ? <button onClick={() => setIsEditingProfile(true)} className="flex items-center gap-2 px-4 py-2 bg-purple-600 text-white rounded-xl hover:bg-purple-700"><FaEdit /> Edit Profile</button> : <div className="flex gap-2"><button onClick={() => setIsEditingProfile(false)} className="px-4 py-2 border rounded-xl">Cancel</button><button onClick={updateProfile} className="flex items-center gap-2 px-4 py-2 bg-indigo-600 text-white rounded-xl"><FaSave /> Save</button></div>}</div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="bg-gray-50 rounded-xl p-5"><h4 className="font-semibold text-purple-600 mb-3">Personal Info</h4><div className="space-y-2"><div className="grid grid-cols-2 gap-3"><div><label className="text-xs text-gray-500">First Name</label>{isEditingProfile ? <input type="text" value={profileData.first_name} onChange={(e) => setProfileData({...profileData, first_name: e.target.value})} className="w-full px-3 py-2 border rounded-lg text-sm" /> : <p className="font-medium">{profileData.first_name || 'Not set'}</p>}</div><div><label className="text-xs text-gray-500">Last Name</label>{isEditingProfile ? <input type="text" value={profileData.last_name} onChange={(e) => setProfileData({...profileData, last_name: e.target.value})} className="w-full px-3 py-2 border rounded-lg text-sm" /> : <p className="font-medium">{profileData.last_name || 'Not set'}</p>}</div></div><div><label className="text-xs text-gray-500">Phone</label>{isEditingProfile ? <input type="tel" value={profileData.phone} onChange={(e) => setProfileData({...profileData, phone: e.target.value})} className="w-full px-3 py-2 border rounded-lg text-sm" /> : <p>{profileData.phone || 'Not set'}</p>}</div></div></div>
                  <div className="bg-gray-50 rounded-xl p-5"><h4 className="font-semibold text-purple-600 mb-3">Account</h4><button onClick={() => setShowPasswordModal(true)} className="flex items-center gap-2 px-4 py-2 border border-purple-600 text-purple-600 rounded-xl hover:bg-purple-50"><FaKey /> Change Password</button></div>
                </div>
              </div>
            </div>
          )}
      ...
      {showWoredaModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto p-6">
            <div className="flex justify-between items-center mb-6"><h2 className="text-xl font-bold text-gray-800">Add Woreda Admin</h2><button onClick={() => setShowWoredaModal(false)} className="p-2 hover:bg-gray-100 rounded-full text-2xl">×</button></div>
            <form onSubmit={handleCreateWoreda} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="md:col-span-2"><input type="text" placeholder="Woreda Name" value={woredaFormData.woreda_name} onChange={(e) => setWoredaFormData({...woredaFormData, woreda_name: e.target.value})} className="w-full px-4 py-3 border rounded-xl text-sm" required /></div>
                <input type="text" placeholder="First Name" value={woredaFormData.first_name} onChange={(e) => setWoredaFormData({...woredaFormData, first_name: e.target.value})} className="w-full px-4 py-3 border rounded-xl text-sm" required />
                <input type="text" placeholder="Middle Name" value={woredaFormData.middle_name} onChange={(e) => setWoredaFormData({...woredaFormData, middle_name: e.target.value})} className="w-full px-4 py-3 border rounded-xl text-sm" />
                <input type="text" placeholder="Last Name" value={woredaFormData.last_name} onChange={(e) => setWoredaFormData({...woredaFormData, last_name: e.target.value})} className="w-full px-4 py-3 border rounded-xl text-sm" required />
                <select value={woredaFormData.gender} onChange={(e) => setWoredaFormData({...woredaFormData, gender: e.target.value})} className="w-full px-4 py-3 border rounded-xl text-sm"><option>Male</option><option>Female</option><option>Other</option></select>
                <input type="number" placeholder="Age" value={woredaFormData.age} onChange={(e) => setWoredaFormData({...woredaFormData, age: e.target.value})} className="w-full px-4 py-3 border rounded-xl text-sm" required />
                <input type="email" placeholder="Email" value={woredaFormData.email} onChange={(e) => setWoredaFormData({...woredaFormData, email: e.target.value})} className="w-full px-4 py-3 border rounded-xl text-sm" required />
                <input type="tel" placeholder="Phone" value={woredaFormData.phone} onChange={(e) => setWoredaFormData({...woredaFormData, phone: e.target.value})} className="w-full px-4 py-3 border rounded-xl text-sm" />
                <input type="password" placeholder="Password" value={woredaFormData.password} onChange={(e) => setWoredaFormData({...woredaFormData, password: e.target.value})} className="w-full px-4 py-3 border rounded-xl text-sm" required minLength="6" />
              </div>
              <div className="flex justify-end gap-3 pt-4"><button type="button" onClick={() => setShowWoredaModal(false)} className="px-5 py-2.5 border rounded-xl">Cancel</button><button type="submit" className="px-5 py-2.5 bg-gradient-to-r from-purple-600 to-indigo-600 text-white rounded-xl">Create Woreda</button></div>
            </form>
          </div>
        </div>
      )}
      ...
      {showReportModal && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto p-6">
            <div className="flex justify-between items-center mb-6"><h2 className="text-xl font-bold text-gray-800"><FaPaperPlane className="inline mr-2 text-purple-500" /> Send New Report</h2><button onClick={() => setShowReportModal(false)} className="p-2 hover:bg-gray-100 rounded-full text-2xl">×</button></div>
            <form onSubmit={handleSendReport} className="space-y-4">
              ...
              <div className="flex justify-end gap-3 pt-4">
                ...
                <button type="submit" className="px-5 py-2.5 bg-gradient-to-r from-purple-600 to-indigo-600 text-white rounded-xl">Send Report</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {showWoredaDetailModal && selectedWoreda && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full p-6">
            <div className="flex justify-between items-center mb-4"><h2 className="text-xl font-bold text-gray-800">Woreda Details</h2><button onClick={() => { setShowWoredaDetailModal(false); setSelectedWoreda(null); }} className="p-2 hover:bg-gray-100 rounded-full text-2xl">×</button></div>
            <div className="flex items-center gap-4 mb-6"><div className="w-16 h-16 bg-purple-100 rounded-2xl flex items-center justify-center text-2xl"><FaMapMarkerAlt className="text-purple-600 text-2xl" /></div><div><h3 className="text-lg font-bold text-gray-800">{selectedWoreda.woreda_name}</h3><p className="text-purple-600 text-sm">Woreda</p></div></div>
            <div className="bg-gray-50 rounded-xl p-4"><div className="grid grid-cols-2 gap-4 text-sm"><div><p className="text-gray-500 text-xs">Admin Name</p><p className="font-medium">{selectedWoreda.admin_name}</p></div><div><p className="text-gray-500 text-xs">Email</p><p className="font-medium">{selectedWoreda.email}</p></div><div><p className="text-gray-500 text-xs">Phone</p><p className="font-medium">{selectedWoreda.phone || 'Not provided'}</p></div><div><p className="text-gray-500 text-xs">Status</p><p className={`font-medium ${selectedWoreda.status === 'active' ? 'text-green-600' : 'text-gray-600'}`}>{selectedWoreda.status}</p></div></div></div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto p-6">
            ...
            <div className="flex gap-3 mt-6 pt-4 border-t">
              <button onClick={() => fetchConversationThread(selectedReport.id)} className="flex-1 px-4 py-2 bg-purple-600 text-white rounded-xl flex items-center justify-center gap-2"><FaComment /> Open Chat</button>
            </div>
          </div>
        </div>
      )}
